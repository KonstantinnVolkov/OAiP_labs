...
begin
  If not (Pos(key,'ÀÁÂÃÄÅ¨ÆÇÈÉÊËÌÍÎÏĞÑÒÓÔÕÖ×ØÙÚÛÜİŞßàáâãäå¸æçèéêëìíîïğñòóôõö÷øùúûüışÿ')>0) then
...
begin
  ...
  if str = '' then
  begin
    ShowMessage('Ïîëå "Òèï" íå ìîæåò áûòü ïóñòûì!');
    exit;
  end
  else
  data.DataType := str;
  ...
  if str = '' then
  begin
    ShowMessage('Ïîëå "Íàèìåíîâàíèå" íå ìîæåò áûòü ïóñòûì!');
    exit;
  end
  else
  data.Name := str;
  ...
  if str = '' then
  begin
    ShowMessage('Ïîëå "Àâòîğ" íå ìîæåò áûòü ïóñòûì!');
    exit;
  end
  else
  data.Author := str;
  ...
  if str = '' then
  begin
    ShowMessage('Ïîëå "Öåíà" íå ìîæåò áûòü ïóñòûì!');
    exit;
  end
  else
  data.Cost := str;
